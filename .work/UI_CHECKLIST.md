...
- [ ] Critical stylesheet UI: interrupt the first `/static/waf.css` request on a mobile viewport; the app must reload it after bounded backoff before rendering, with a 74 px sidebar and no natural-size logo covering page controls.
- [ ] Critical stylesheet network: verify a failed critical CSS request is followed by a same-origin request carrying `style_retry`, while two consecutive failures stop bootstrap instead of exposing a partially styled application.
...
- [ ] Services UI: create a disposable Easy service, open its editor, select the independently mounted Anti-Bot tab, and verify cookie, JavaScript, and CAPTCHA template previews open or notify correctly on desktop and mobile.
- [ ] CI failure report: intentionally fail one browser assertion and verify `pre-security:failure-gate` shows the failed Playwright test title, error text, and concise nearby log context without artifact-upload tail output; a skipped test must be reported as a separate blocker.
...
- [ ] Rootless UI image: build the E2E stack after setting `ui/nginx.conf` and `ui/nginx.rootless.conf` source modes to `0600`; verify the UI runs as `101:101`, both image configs are `0644 root:root`, `nginx -t` succeeds, and runtime health remains available.
- [ ] Existing-install upgrade permissions: before rebuilding, set legacy UI configs/static assets/scripts to `0600` and `.env` to `0644`; run the installer repair and verify configs/assets become readable, scripts executable, `.env` returns to `0600`, private keys remain private, and the post-upgrade runtime health gate passes.
