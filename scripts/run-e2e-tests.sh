...
if [ -n "${CI_CONCURRENT_ID:-}" ]; then
  ...
  case "$E2E_CI_RUNNER_ID" in
    *[!0-9]*|'') echo "[e2e] ERROR: CI_RUNNER_ID must be numeric" >&2; exit 1 ;;
  esac
...
WAF_BROWSER_RUNTIME_URL="http://e2e-management.test"
...
if [ "$E2E_BROWSER_ONLY" = "1" ]; then
  ...
  docker run --rm --network "$WAF_BROWSER_DOCKER_NETWORK" --user "$(id -u):$(id -g)" \
    ...
    -e WAF_E2E_DISPOSABLE -e WAF_BROWSER_BASE_URL -e WAF_E2E_RUNTIME_URL="$WAF_BROWSER_RUNTIME_URL" \
    -e WAF_E2E_USERNAME -e WAF_E2E_PASSWORD -e WAF_E2E_RUN_ID \
