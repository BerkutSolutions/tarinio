...
test("requests.real-backend-failure-keeps-shell", async ({ authenticatedPage: page }, testInfo) => {
  ...
  const syncFile = process.env.WAF_BROWSER_FAULT_SYNC_FILE;
  if (!syncFile) throw new Error("WAF_BROWSER_FAULT_SYNC_FILE is required for the real runtime-fault workflow");
