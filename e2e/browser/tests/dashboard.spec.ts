...
test("dashboard.widgets", async ({ authenticatedPage: page }) => {
  ...
  for (const widget of ["services", "traffic-summary", "containers-health", "top-ips", "top-countries", "requests-series", "top-urls"]) {
...
test("dashboard.layout-reset-resize", async ({ authenticatedPage: page }) => {
  ...
  const frame = page.locator('[data-widget-id="services"]');
  ...
  if ((page.viewportSize()?.width || 0) > 900) {
    ...
    await expect.poll(() => page.evaluate(() => {
      ...
      return Number(persisted.find((item: { id?: string }) => item.id === "services")?.width || 0);
    })).toBeGreaterThan(340);
    await openPage(page, "/dashboard", page.locator('[data-widget-id="services"]'));
    await expect(page.locator('[data-widget-id="services"]')).toHaveCSS("width", `${Math.round(after?.width || 0)}px`);
  } else {
