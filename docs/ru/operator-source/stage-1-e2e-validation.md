...
| Bootstrap первого администратора | первый user создаётся только через bootstrap flow | `POST /core-docs/api/auth/bootstrap` создаёт `admin` | PASS |
...
| Login after setup | оператор логинится стандартным путём | `POST /core-docs/api/auth/login` успешен | PASS |
