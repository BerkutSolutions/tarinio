...
### Вкладка 9 — WebSocket инспекция (11 тестов)
...
Preflight в `scripts/release.sh` запускает все tab-тесты до основного `go test ./...` и до меню выбора версии. Падение хотя бы одного теста прерывает релиз с кодом 1.
