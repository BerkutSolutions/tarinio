...
TARINIO `1.3.5` включает 5 встроенных security-профилей сервиса:
...
## Дополнительные настройки профиля (v1.3.x)

Помимо базовых preset-значений, все профили поддерживают следующие группы расширенных настроек:

### mTLS (входящий)
- `front_service.mtls_enabled` — требовать клиентский сертификат.
- `front_service.mtls_optional` — опциональный режим (передавать статус без блокировки).
- `front_service.mtls_verify_depth` — глубина проверки цепочки (по умолчанию `1`).
- `front_service.mtls_client_ca_ref` — путь к CA-файлу клиентских сертификатов.
- `front_service.mtls_pass_headers` — передавать заголовки `X-Client-Verify` / `X-Client-DN` в upstream.

### Upstream mTLS (исходящий)
- `upstream_routing.upstream_mtls_enabled` — использовать клиентский сертификат при проксировании.
- `upstream_routing.upstream_mtls_cert_ref` — путь к сертификату WAF как клиента.
- `upstream_routing.upstream_mtls_key_ref` — путь к приватному ключу клиента.
- `upstream_routing.upstream_mtls_ca_ref` — CA для верификации upstream-сервера.

### JA3/JA4 Fingerprinting
- `security_behavior_and_limits.blacklist_ja3` — список JA3-fingerprint для блокировки.
- `security_behavior_and_limits.blacklist_ja3_urls` — URL-источники JA3-blacklist.

### Virtual Patching
- `virtual_patches[]` — временные правила блокировки с TTL: `match_uri`, `match_header`, `match_body`, `action`, `expires_at`.

### Geo/Time Windows
- `geo_time_windows[]` — ограничения по GeoIP и расписанию: `countries`, `weekdays`, `time_from`, `time_to`, `action`.

### WebSocket Inspection
- `security_websocket.use_ws_inspection` — включить инспекцию WS-фреймов.
- `security_websocket.ws_block_patterns` — regex-паттерны для блокировки.
- `security_websocket.ws_max_message_bytes` — лимит размера фрейма.
- `security_websocket.ws_rate_msg_per_sec` — лимит частоты фреймов.
