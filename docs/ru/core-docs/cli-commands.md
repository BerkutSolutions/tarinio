...
```powershell
.\waf-cli.ps1 api GET /core-docs/api/sites
.\waf-cli.ps1 api GET /core-docs/api/reports/revisions
.\waf-cli.ps1 api PUT /core-docs/api/anti-ddos/settings --file .\antiddos.json
.\waf-cli.ps1 api DELETE /core-docs/api/sites/site-a
```
...
Во время сборки образа `ui` автоматически выполняется `go test ./core-docs/ui/tests`, поэтому ошибки UI/i18n останавливают запуск профиля.
...
docker compose exec control-plane wget -qO- http://127.0.0.1:8080/core-docs/api/requests
```
