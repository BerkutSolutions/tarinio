...
В версии `2.0.11` TARINIO включает полноценный набор средств наблюдаемости для рабочих и высокодоступных развёртываний.

## Что входит в контур наблюдаемости

- совместимые с Prometheus метрики от control-plane;
- совместимые с Prometheus метрики от загрузчика рантайма;
- экспортёры PostgreSQL и Redis в лаборатории высокой доступности;
...
- время ожидания и захвата блокировок высокой доступности;
- выполнение задач, доступных только лидеру;
- результаты перезагрузки рантайма;
- сведения о сборке и версии.

### Метрики рантайма

- количество HTTP-запросов и задержки в API загрузчика;
- число перезагрузок рантайма;
...
- отслеживать поведение кластера высокой доступности;
...
- при подозрении на деградацию рантайма;
