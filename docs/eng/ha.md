...
TARINIO `2.0.11` now supports a practical multi-node control-plane topology on one Docker host:
...
The supported HA shape in `2.0.11` is:
...
HA apply operations where the runtime process is intentionally supervised by the
launcher.
...
Validated HA lab resource envelope:
...
Validated on the bundled HA lab:

- 20 demo tenants provisioned successfully through `api-lb`
- load-balanced `/api/app/meta` requests alternated between `control-plane-a` and `control-plane-b`
...
This is a serious HA improvement, but it is important to be precise:

- control-plane HA is implemented;
