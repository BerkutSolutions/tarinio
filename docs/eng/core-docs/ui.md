...
This document describes the real TARINIO administrative interface for version `current release`: login, onboarding, healthcheck, the sidebar sections, and the practical workflows behind each screen.
...
1. Create the first administrator through `POST /core-docs/api/auth/bootstrap`.
...
When a module supports self-healing, healthcheck can trigger `POST /core-docs/api/app/compat/fix`.
...
- Let' Encrypt staging and wildcard options.
...
This section manages global Anti-DDoS settings through `/core-docs/api/anti-ddos/settings`.
...
This is the core change-management screen in `current release`.
...
- clearing the status timeline without losing the revision' pinned last apply result.

The screen is powered by the aggregated revision catalog `GET /core-docs/api/revisions`.
...
The `/activity` section exposes administrative audit data through `GET /core-docs/api/audit`.
