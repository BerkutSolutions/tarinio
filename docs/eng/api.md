...
This document describes the current control-plane HTTP API for version `2.0.8`. The catalog is aligned with the routes registered in `control-plane/internal/httpserver/server.go`.
...
The new aggregated revision catalog in `2.0.8`.
