...
This document explains how TARINIO `2.0.11` handles secrets, what `Vault` is responsible for, which data is allowed to stay local, and what operators should verify after a fresh install or upgrade.
...
In `2.0.11`, the product should no longer look like a stack where backend passwords simply live in `.env` and spread across containers without a controlled secret flow.
...
This is the default and preferred `2.0.11` mode:
...
## What Counts As Normal In `2.0.11`
...
If those conditions hold, the secret-management model in `2.0.11` is current, predictable, and safe enough for real operation.
