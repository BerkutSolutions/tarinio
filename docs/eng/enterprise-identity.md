...
This document describes the real enterprise identity model in TARINIO `2.0.9`.

## What Is Implemented

TARINIO `2.0.9` provides:
...
TARINIO `2.0.9` does **not** expose a standalone direct `LDAP` password-login endpoint.
...
1. The login page discovers whether enterprise SSO is enabled through `GET /api/auth/providers`.
2. When enabled, the UI shows the enterprise SSO button.
3. `GET /api/auth/oidc/start` creates a short-lived login challenge and redirects the browser to the provider authorization endpoint.
4. `GET /api/auth/oidc/callback` exchanges the authorization code, validates the `id_token`, checks issuer/audience/nonce, and creates a TARINIO session.
