...
## Key Domains In 2.0.6

The main architectural domains in version `2.0.6` are:
...
In `2.0.6`, the revision subsystem includes:
...
In `2.0.6`, the UI is not a thin shell over a few backend objects. It reflects multiple operational layers:
...
Those documents define the foundation, while this wiki maps that foundation to the real product surface in `2.0.6`.
