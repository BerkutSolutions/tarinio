...
- CI browser shards are serialized on the shared shell runner, and Dashboard Docker metrics are scoped to the current Compose project; concurrent disposable stacks can no longer contaminate each other's container rows or exhaust the runner during long UI suites.
