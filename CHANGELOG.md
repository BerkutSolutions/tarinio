...
- OWASP ZAP now scans through its disposable DAST stack's project-local network and uses a writable ephemeral home under the runner UID, preventing scanner-daemon startup failures from concurrent Docker network activity or rootless Selenium cache initialization.
