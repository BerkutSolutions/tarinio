...
- Added an isolated default-profile health regression job. It renders a temporary copy of `deploy/compose/default` with disposable names and an init-only volume-permission override, so the production stack remains untouched while sentinel write paths are tested.
