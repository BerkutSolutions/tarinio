...
### Тесты / Кодировка
- Добавлен обязательный репозиторный тест кодировки `TestRepositoryTextFilesEncoding` (`ui/tests/repo_encoding_test.go`), который проверяет все текстовые файлы на:
  - валидный UTF-8;
  - отсутствие UTF-8 BOM;
  - отсутствие маркеров mojibake/поврежденной кодировки.
- Тест запускается в `go test ./ui/tests` и, как следствие, блокирует сборку UI-образа при любой проблеме кодировки.
...
### Переводы
- Исправлены проблемные битые символы в шаблонах 403/429.
