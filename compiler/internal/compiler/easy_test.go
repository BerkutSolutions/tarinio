...
func TestRenderEasyArtifacts_AdminBypassOnlyForManagementSites(t *testing.T) {
	...
	if strings.Contains(siteConf, `waf_antibot_exclusion_match ~* "^(?:GET|HEAD):/static/"`) {
		t.Fatalf("static assets must not bypass antibot automatically, got: %s", siteConf)
	}
...
func TestRenderEasyArtifacts_UsesSafeTemplateWhenProfileMissing(t *testing.T) {
	...
	if !strings.Contains(siteConf, "if ($request_method ~ ^(GET|POST|HEAD|OPTIONS|PUT|PATCH|DELETE)$)") || !strings.Contains(siteConf, "if ($waf_easy_method_allowed = 0)") {
