...
func TestUIContract_LoginAppearanceUsesSharedThemeStyles(t *testing.T) {
	...
	if !strings.Contains(contents["theme script"], "window.requestAnimationFrame") {
		t.Fatal("login appearance must wait for the themed layout before it becomes visible")
	}
