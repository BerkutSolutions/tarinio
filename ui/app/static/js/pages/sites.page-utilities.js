export const TLS_EXPIRY_ALERT_STORAGE_KEY = "waf_tls_expiry_alerted";
...
export function readExpiryAlertState() {
  try {
...
export function writeExpiryAlertState(state) {
  try {
...
export function mergeProfilesBySite(primary, secondaryPayload, normalizeSiteID) {
  const map = new Map();
  for (const item of normalizeArray(primary)) {
