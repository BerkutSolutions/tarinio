...
export function normalizeAuthBasicUsers(value) {
  const seen = new Set();
...
function renderAuthPasswordInput(user, index) {
...
export function renderAuthUsersEditor(users, ctx) {
  const safeUsers = normalizeAuthBasicUsers(users, normalizeArray);
  return `
              ...
              <tr>
                ...
                <td>
                  <div class="waf-auth-password-cell">
                    ${renderAuthPasswordInput(user, index)}
...
export function renderAuthSessionTtlOptions(ttlMinutes, ctx) {
