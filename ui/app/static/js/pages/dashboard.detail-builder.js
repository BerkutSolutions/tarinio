...
function buildWidgetDetail(action, payload, stats, detailModel, containersOverview, ctx, deps) {
  ...
  if (action === "services-up" || action === "services-down") {
    ...
    return {
      ...
      ], ctx, deps) + renderDetailTable(rows, ctx, ctx.t("dashboard.detail.component"), ctx.t("dashboard.detail.state"), {}, deps)
    };
  }
  ...
  if (action === "top-countries" || action === "country-detail") {
    ...
    const rows = code && code !== "UNK" ? (detailModel?.attacksByCountry || []).filter((item) => deps.normalizeCountryCode(item.key) === code) : (detailModel?.attacksByCountry || []);
    return {
      title: code && code !== "UNK" ? `${ctx.t("dashboard.detail.country")} ${countryName(code, deps)}${countryFlag(code, deps) ? ` (${countryFlag(code, deps)})` : ""}` : ctx.t("dashboard.widget.topCountries"),
      subtitle: ctx.t("dashboard.detail.countrySubtitle"),
      body: renderDetailTable(rows, ctx, ctx.t("dashboard.detail.country"), ctx.t("dashboard.detail.attacks"), { labelFormatter: (item) => deps.renderCountryBadge(item.key) }, deps)
    };
  }
  ...
  if (action === "containers-health") {
    ...
    return {
      ...
      ], ctx, deps) + renderDetailTable(rows, ctx, ctx.t("dashboard.containers.container"), ctx.t("dashboard.containers.cpu"), {
        labelFormatter: (item) => `
