// T3-4 extracted resource upsert flow.
export async function resolveACMEAccountEmail(draft, ctx, deps) {
  const fromDraft = deps.normalizeEmail(draft?.acme_account_email);
  if (deps.isValidEmail(fromDraft)) {
    return fromDraft;
  }
  ...
  if (siteID) {
    try {
      ...
      const ownEmail = deps.normalizeEmail(ownProfile?.front_service?.acme_account_email);
      if (deps.isValidEmail(ownEmail)) {
        return ownEmail;
      }
    } catch (error) {
      if (error?.status !== 404) {
  ...
  try {
    ...
    for (const site of deps.normalizeArray(sites)) {
      ...
      try {
        ...
        const email = deps.normalizeEmail(profile?.front_service?.acme_account_email);
  ...
  try {
    ...
    const sessionEmail = deps.normalizeEmail(me?.email || me?.user?.email);
    if (deps.isValidEmail(sessionEmail)) {
      return sessionEmail;
    }
  } catch (error) {
...
export async function upsertSiteResources(draft, ctx, existingSite, existingUpstream, existingTLSConfig, options = {}, deps) {
  ...
  const siteExists = async (siteID) => {
    ...
    return Array.isArray(sites) && sites.some((site) => deps.normalizeSiteID(site?.id) === deps.normalizeSiteID(siteID));
  };
  const upstreamExists = async (upstreamID) => {
    const upstreams = await ctx.api.get("/api/upstreams", requestOptions);
    return Array.isArray(upstreams) && upstreams.some((upstream) => deps.normalizeSiteID(upstream?.id) === deps.normalizeSiteID(upstreamID));
  };
  const tlsConfigMatches = async (siteID, certificateID) => {
    ...
    return tlsConfigs.some((item) =>
      deps.normalizeSiteID(item?.site_id) === deps.normalizeSiteID(siteID) &&
  ...
  try {
    ...
    if (draft.tls_enabled) {
      ...
      if (!importMode && !hadCertificateBefore && !isCurrentTLSCertificate) {
        ...
        const acmeAccountEmail = draft.tls_self_signed ? "" : await resolveACMEAccountEmail(draft, ctx, deps);
