import { findEasyProfile, normalizeArray, tryGetJSON } from "./sites.routing-merge.js";
import {
...
import { defaultSiteDraft, siteDraftFromData } from "./sites.draft-core.js";

export function applyEasyProfileToDraft(draft, profile) {
  if (!profile || typeof profile !== "object") return draft;
  ...
  return {
    ...
    ban_escalation_scope: BAN_SCOPE_VALUES.includes(String(security.ban_escalation_scope || "").trim().toLowerCase()) ? String(security.ban_escalation_scope || "").trim().toLowerCase() : draft.ban_escalation_scope,
    ...
    antibot_challenge_template: antibot.antibot_challenge_template || draft.antibot_challenge_template || "v2",
    antibot_uri: antibot.antibot_uri || draft.antibot_uri,
    antibot_session_inactivity_minutes: normalizeAuthSessionTTLMinutes(antibot.session_inactivity_minutes ?? draft.antibot_session_inactivity_minutes),
    ...
    antibot_exclusion_rules: normalizeAntibotExclusionRules(antibot.exclusion_rules || draft.antibot_exclusion_rules),
    ...
    allow_local_country_ips: Boolean(country.allow_local_ips ?? draft.allow_local_country_ips),
    show_geo_block_page: Boolean(country.show_geo_block_page ?? draft.show_geo_block_page),
    use_custom_error_pages: Boolean(profile.use_custom_error_pages ?? draft.use_custom_error_pages ?? true),
    disabled_error_pages: Array.isArray(profile.disabled_error_pages) ? profile.disabled_error_pages : (Array.isArray(draft.disabled_error_pages) ? draft.disabled_error_pages : []),
